...
use crate::CaptchaId;
use cxlib_imageproc::find_max_ncc;
use log::{debug, warn};
...
pub fn auto_solve_captcha(
    ...
    let small_img = cxlib_imageproc::download_image(&agent, &cutout_image_url)?;
    let big_img = cxlib_imageproc::download_image(&agent, &shade_image_url)?;
    let max_x = cxlib_imageproc::find_sub_image(&big_img, &small_img, find_max_ncc);
    debug!("本地滑块结果：{max_x}");
...
pub fn captcha_solver(agent: &ureq::Agent, captcha_id: &str) -> Result<String, cxlib_error::Error> {
    ...
    Err(cxlib_error::Error::CaptchaEmptyError)
}
