...
// 登录页
pub fn login_page(client: &Agent) -> Result<Response<Body>, AgentError> {
    Ok(client.get(&ProtocolItem::LoginPage.to_string()).call()?)
}

// 非明文密码登录
pub fn login_enc(client: &Agent, uname: &str, pwd_enc: &str) -> Result<Response<Body>, AgentError> {
    Ok(client
...
// 账号设置页
pub fn account_manage(client: &Agent) -> Result<Response<Body>, AgentError> {
    Ok(client
