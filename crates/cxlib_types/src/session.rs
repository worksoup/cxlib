    ...
    UserProtocol: UserProtocolTrait,
{
    ...
    #[inline]
    pub fn load_cookies<P: AsRef<Path>>(uname: &str, cookies_file: P) -> Result<Self, LoginError> {
        let agent = Self::load_cookies_raw(cookies_file)?;
    ...
    UserProtocol: UserProtocolTrait + 'static,
{
    ...
    pub fn load_cookies_or_relogin<P: AsRef<Path>>(
        enc_passwd: &str,
        uname: &str,
        cookies_file: P,
        login_solver: &UntypedLoginSolver<UserProtocol>,
    ) -> Result<Self, LoginError> {
        match Self::load_cookies(uname, cookies_file.as_ref()) {
            Ok(s) => Ok(s),
            Err(e) => match e {
                LoginError::LoginExpired(_) => {
                    Self::relogin(uname, enc_passwd, cookies_file, login_solver)
                }
    ...
    pub fn relogin<P: AsRef<Path>>(
        ...
        Self::store_cookies(&agent, store_path)?;
        let session = Self::from_raw(uname.to_string(), agent, cookies)?;
...
impl<U> Session<U> {
    ...
    #[inline]
    pub fn store_cookies<P: AsRef<Path>>(agent: &Agent, store_path: P) -> Result<(), LoginError> {
