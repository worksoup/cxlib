...
impl Photo {
    async fn find_object_id(
        ...
        let r = protocol::pan_chaoxing(session).await?;
    ...
    pub async fn default(session: &Session) -> Option<Self> {
        Self::get_from_cxpan(session, |a| a == "1.png" || a == "1.jpg").await
    }
    pub async fn get_from_cxpan(session: &Session, p: impl Fn(&str) -> bool) -> Option<Self> {
        let object_id = Self::find_object_id(session, p)
            .await
            .unwrap();
    ...
    pub async fn get_from_file(session: &Session, file_path: impl AsRef<Path>) -> Self {
