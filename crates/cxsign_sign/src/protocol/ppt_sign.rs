...
pub fn general_sign(session: &Session, active_id: &str) -> Result<Response, Box<ureq::Error>> {
    ...
    session.get(&format!("{}?activeId={active_id}&uid={uid}&clientip=&latitude=-1&longitude=-1&appType=15&fid={fid}&name={stu_name}", ProtocolEnum::PptSign)).call().map_err(|e| e.into())
}
...
pub fn photo_sign(
    ...
    let stu_name = session.get_stu_name();
    // NOTE 存疑。
    session.get(&format!("{}?activeId={active_id}&uid={uid}&clientip=&useragent=&latitude=-1&longitude=-1&appType=15&fid={fid}&objectId={object_id}&name={}", ProtocolEnum::PptSign, percent_encoding::utf8_percent_encode(stu_name, percent_encoding::NON_ALPHANUMERIC))).call().map_err(|e| e.into())
}
...
pub fn location_sign_url(
    ...
    format!("{}?name={stu_name}&address={address}&activeId={active_id}&uid={uid}&clientip=&latitude={lat}&longitude={lon}&fid={fid}&appType=15&ifTiJiao={if_tijiao}", ProtocolEnum::PptSign)
}
...
pub fn signcode_sign(
    ...
    let fid = session.get_fid();
    let stu_name = session.get_stu_name();
    let url = format!("{}?activeId={active_id}&uid={uid}&clientip=&latitude=-1&longitude=-1&appType=15&fid={fid}&name={stu_name}&signCode={signcode}", ProtocolEnum::PptSign);
    Ok(session.get(&url).call()?)
}
