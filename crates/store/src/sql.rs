...
impl DataBase {
    ...
    pub fn 添加位置_失败后则<O: Fn(&DataBase, i64, i64, &Location)>(
        &self,
        位置id: i64,
        course_id: i64,
        位置: &Location,
    ...
    pub fn 获取所有位置(&self) -> HashMap<i64, (i64, Location)> {
        ...
        for c in query.iter() {
            if let Ok(row) = c {
                ...
                位置列表.insert(位置id, (课程号, Location::new(地址, 经度, 纬度, 海拔)));
            } else {
                eprintln!("位置解析行出错：{c:?}.");
            }
        }
    ...
    pub fn 获取为某id的位置(&self, 位置id: i64) -> (i64, Location) {
        let mut query = self
            .connection
        ...
        (courseid, Location::new(addr, lon, lat, alt))
    }
    pub fn 获取特定课程的位置和其id(&self, course_id: i64) -> HashMap<i64, Location> {
        let mut query = self
            .connection
        ...
        for c in query.iter() {
            if let Ok(row) = c {
                ...
                let 经度 = row.read("lon");
                let 海拔 = row.read("alt");
                位置列表.insert(位置id, Location::new(地址, 经度, 纬度, 海拔));
            } else {
                eprintln!("位置解析行出错：{c:?}.");
            }
        }
    ...
    pub fn 获取特定课程的位置(&self, course_id: i64) -> Vec<Location> {
        let mut query = self
            .connection
            .prepare("SELECT * FROM pos WHERE courseid=?;")
            .unwrap();
        ...
        for c in query.iter() {
            if let Ok(row) = c {
                ...
                let lon = row.read("lon");
                let alt = row.read("alt");
                位置列表.push(Location::new(addr, lon, lat, alt));
            } else {
                eprintln!("位置解析行出错：{c:?}.");
            }
        }
...
impl DataBase {
    ...
    pub fn 获取为某别名的位置(&self, alias: &str) -> Option<Location> {
